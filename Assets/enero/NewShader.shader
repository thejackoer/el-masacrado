...
Shader "Shader Forge/NewShader" {
    ...
    SubShader {
        ...
        Pass {
            ...
            uniform float _speccularintensity;
            uniform sampler2D _node_4191; uniform float4 _node_4191_ST;
            uniform float _tesalation;
            uniform fixed _ligthemision;
            struct VertexInput {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float3 _node_4191_var = UnpackNormal(tex2D(_node_4191,TRANSFORM_TEX(i.uv0, _node_4191)));
                float3 normalLocal = _node_4191_var.rgb;
...
////// Emissive:
                ...
                float3 emissive = lerp( UNITY_LIGHTMODEL_AMBIENT.rgb, (node_544*UNITY_LIGHTMODEL_AMBIENT.rgb), _ligthemision );
                float node_7782 = max(0,dot(lightDirection,normalDirection)); // Lambert
                float3 finalColor = emissive + (((node_544*node_7782)+(node_7782*pow(max(0,dot(normalDirection,halfDirection)),exp2(lerp(1,11,_speccularintensity)))*_SpecColor.rgb))*_LightColor0.rgb*attenuation);
...
////// Lighting:
                ...
                float3 node_544 = (_Diffuse_var.rgb*_Color.rgb); // Diffuse Color
                float node_7782 = max(0,dot(lightDirection,normalDirection)); // Lambert
                float3 finalColor = (((node_544*node_7782)+(node_7782*pow(max(0,dot(normalDirection,halfDirection)),exp2(lerp(1,11,_speccularintensity)))*_SpecColor.rgb))*_LightColor0.rgb*attenuation);
